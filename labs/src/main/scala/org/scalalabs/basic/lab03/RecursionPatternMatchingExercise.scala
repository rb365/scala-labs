...
object RecursionPatternMatchingExercise {
  ...
  def checkValuesIncrease(seq: Seq[Int]): Boolean = seq match {
    case seq: Seq[Int] if seq.length <= 1 => true
    case seq: Seq[Int] if seq(0) < seq(1) => checkValuesIncrease(seq.drop(1))
    case _ => false
  }
  ...
  def groupConsecutive[T](in: List[T]): List[List[T]] = in match {
    case in:List[T] if in.isEmpty => List[List[T]]()
    case _ => in.takeWhile(_ == in(0)) :: groupConsecutive(in.dropWhile(_ == in(0)))
  }
  ...
  def groupEquals[T](in: List[T]): List[List[T]] = in match {
    case in:List[T] if in.isEmpty => List[List[T]]()
    case _ => in.filter(_ == in(0)) :: groupEquals(in.filter(_ != in(0)))
  }
  ...
  def compress[T](in: List[T]): List[T] = in match {
    case in:List[T] if in.isEmpty => List[T]()
    case _ => in(0) :: compress(in.dropWhile(_ == in(0)))
  }
  ...
  def amountEqualMembers[T](in: List[T]): List[(Int, T)] = in match {
    case in:List[T] if in.isEmpty => List[(Int, T)]()
    case _ => (in.filter(_ == in(0)).size, in(0)) :: amountEqualMembers(in.filter(_ != in(0)))
  }
  ...
  def zipMultiple(in: List[List[_]]): List[List[_]] = in match {
    case in: List[List[_]] if in.isEmpty => List[List[_]]()
    case in: List[List[_]] if in.length == 1 => in(0).map(List(_))
    case _ => in(0).zip(zipMultiple(in.drop(1))).map(t => t._1 :: t._2)
  }
